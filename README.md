...
### Q: 图表显示不出来，提示"加载数据失败，请重试"？
A: 这通常是因为无法访问外网 CDN（如政府版 Windows 环境）。解决方法：
1. 确保使用最新版本（v1.0.1 及以上），Chart.js 库已包含在本地文件中
2. 如果仍有问题，请检查 `static/js/chart.umd.min.js` 文件是否存在
3. 重新打包应用，确保 `static` 目录下的所有文件都被正确打包
...
### Q: 导出数据时如何自定义保存位置？
A: 浏览器下载行为由浏览器设置控制。如需每次选择保存位置，请在Chrome浏览器中设置：
1. 点击右上角三个点 ⋮ → "设置"
2. 在左侧菜单点击"下载内容"
3. 开启"**下载前询问每个文件的保存位置**"
...
### v1.0.2 (2025-12-24)
- **修复导出路径问题**：使用系统临时目录解决PyInstaller打包后的路径不存在问题
- **新增下载设置说明**：在README中添加如何自定义浏览器下载位置的操作指南

### v1.0.1 (2025-11-28)
- **修复图表显示问题**：将 Chart.js 库从 CDN 改为本地文件，解决在政府版 Windows 等无法访问外网的环境下图表无法显示的问题
- 图表库文件已包含在打包文件中，无需网络连接即可正常显示图表
